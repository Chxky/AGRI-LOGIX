...
const Sidebar: React.FC<SidebarProps> = ({ collapsed }) => {
  ...
  const menuItems = [
    ...
    { key: '/register-farmer', icon: <UserAddOutlined />, label: 'Register Farmer' },
    { key: '/ward', icon: <EnvironmentOutlined />, label: 'Ward Overview' },
    { key: '/verify', icon: <ScanOutlined />, label: 'Scan & Verify' },
    { key: '/flag', icon: <FlagOutlined />, label: 'Flag Bag' },
  ];
      ...
      <div style={{ position: 'absolute', bottom: 0, width: '100%', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
        {!collapsed && (
          <div style={{ padding: '8px 20px', color: '#667788', fontSize: 10, textAlign: 'center' }}>
            <div>Ward-Level Operations</div>
            <div>Field Agent Terminal</div>
          </div>
        )}
        <Menu
          theme="dark"
